...
describe('app/filesystem.js', function() {
    ...
    describe('init()', function() {
        it('Should should return false if there is an error', async () => {
...
describe.only('app/browser.js', function () {
    ...
    describe('triggerEvents()', function() {
        ...
        it('Should return false if no triggers are defined', async () => {
            const _browser = await puppeteer.launch({ignoreHTTPSErrors: true});
            let page = await browser.newPage(_browser, config.viewport('test',1024,768));
            let result = await browser.triggerEvents([],"pre",page,"google","homepage","desktop",settings);
            await _browser.close();
            assert.strictEqual(
                result,
                false
        ...
        it('Should return true if no warnings were encountered', async () => {
            ...
            let result = await browser.triggerEvents([config.trigger('click','#main','pre')],"pre",page,"google","homepage","desktop",settings);
            await _browser.close();
            assert.strictEqual(
                result,
                true
        ...
        it('Should return warning if selector is null', async () => {
            ...
            let page = await browser.newPage(_browser, config.viewport('test',1024,768));
            await browser.gotoPath('https://www.google.com',page);
            let result = await browser.triggerEvents([config.trigger('click','#facebook','pre')],"pre",page,"google","homepage","desktop",settings);
            await _browser.close();
